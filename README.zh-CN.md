...
1. **下载**：打开本仓库的 [Releases 发布页](../../releases)，两种包任选其一：
   - **精简版（推荐）**——压缩包更小（约 150 MB）。首次运行会出现安装向导，自动检查环境并下载缺失组件（几分钟，仅一次）。
   - **完整版**——全部内置（约 250 MB）。完全离线可用，无需首次配置。
...
```
① 环境检查 ─ 系统已装 Node.js ≥ 22.15？→ 直接复用，什么都不用下载
② 运行时   ─ 没有？自动从你所在地区的镜像下载 Node.js（约 34 MB）
③ 组件安装 ─ 通过 npm 安装 dsh 及依赖（仅首次）
④ 启动     ─ 主界面打开
```

下载源根据**时区自动判定**：中国时区走 npmmirror 镜像，其他地区走国际官方源；向导里也可以手动切换镜像（某个源慢的时候有用）。每一步都有进度条，失败会显示**重试**按钮。
...
| 每次启动都要检查/下载吗？ | 不用——只有缺东西才会触发。之后启动直达界面（约 2 秒） |
...
下载新的 release ZIP → 解压 → 用新文件夹替换旧文件夹即可。聊天记录存在应用文件夹**外面**，不受影响。（精简版首次运行新版本时会重新检查，只下载有变化的部分。）
