...
1. **下载**：打开本仓库的 [Releases 发布页](../../releases)，下载 ZIP 压缩包（约 138 MB）。首次运行会出现安装向导，自动检查环境并下载缺失组件（几分钟，仅一次）。
...
下载新的 release ZIP → 解压 → 用新文件夹替换旧文件夹即可。聊天记录存在应用文件夹**外面**，不受影响。（新版本首次运行时会重新检查，只下载有变化的部分。）
